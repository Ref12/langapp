...
| `conversation.generateVoiceTurn` | Generate validated English/target-locale segments for the bilingual voice tutor |
| `conversation.suggestTitle` | Suggest a short thread title |

### Implemented voice operation

`conversation.generateVoiceTurn` uses the existing independently configured OpenAI-compatible transport, with no Azure dependency. Input is the active target language and up to 24 completed recent messages. The voice history helper bounds history to 24,000 characters before schema validation; failed/pending placeholders are excluded and each message is capped at 20,000 characters. Empty history requests a short English greeting. Later prompts permit context-sensitive English/target mixtures and explicit learner preferences.

Output is 1–12 ordered `{ text, locale }` segments (at most 2,000 characters each). Runtime validation rejects unknown locales, other profiles' target locales, non-English opening segments and empty text. Canonical content is joined directly from the returned segments. It is not passed through English-only weaving. Acoustic measurements are never generated by this operation.

Microphone transcription and scripted assessment use the separate narrow services in `src/core/voice/services.ts` and contracts in `contracts.ts`. Browser playback implements a separate locale-aware playback contract in `src/core/speech.ts`. A backend/token broker can replace the transport adapters without changing the voice review/Send policy.
