...
describe('isolated named browser profiles', () => {
  it.each([4, 6])('upgrades a v%s workspace in place without changing any learning or credentials', async version => {
    ...
    const newTables = version === 4 ? ['profileState', 'mahjongGames', 'characterStates', 'libraryBooks', 'sudokuGames'] : ['characterStates', 'libraryBooks', 'sudokuGames']
    ...
    expect(database.db.verno).toBe(9)
