...
it('persists entries and scratch markers without changing learning, rejecting stale edits', async () => {
  const learning = await loadWorkspace(), game = create(), index = game.givens.indexOf(0)
  await db.sudokuGames.put(game)
  const next = await updateSudoku(game, { type: 'exclude', index, value: 3 })
  expect((await db.sudokuGames.get('current'))?.excluded[index]).toBe(4)
  await expect(updateSudoku(game, { type: 'place', index, value: 1 })).rejects.toThrow('another tab')
  await updateSudoku(next, { type: 'place', index, value: game.solution[index] })
  expect((await db.sudokuGames.get('current'))?.values[index]).toBe(game.solution[index])
...
it('surfaces failed saves and isolates puzzles between profiles', async () => {
  ...
  await expect(updateSudoku(game, { type: 'place', index: game.givens.indexOf(0), value: 1 })).rejects.toThrow('Storage full')
