...
describe('Hear voice settings', () => {
  ...
  it('explains missing browser support and empty language lists without requesting speech', async () => {
    ...
    act(() => vi.advanceTimersByTime(3000))
    expect(screen.getByText(/No Mandarin voices are listed by this browser/)).toBeInTheDocument()
    expect(screen.getByText(/No English voices are listed by this browser/)).toBeInTheDocument()
    expect(vi.getTimerCount()).toBe(0)
    expect(synthesis.speak).not.toHaveBeenCalled()
    expect(fetch).not.toHaveBeenCalled()
  })

  it('keeps Automatic previews usable in the actual app when Android lists no voices', async () => {
    voices = []
    render(<App />)
    expect(await screen.findByLabelText('Mandarin voice')).toHaveValue('')
    expect(screen.getByLabelText('English voice')).toHaveValue('')
    expect(synthesis.speak).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Test Mandarin voice' }))
    expect(synthesis.speak).toHaveBeenCalledOnce()
    expect(synthesis.speak.mock.calls[0][0].voice).toBeUndefined()
    expect(synthesis.speak.mock.calls[0][0].lang).toBe('zh-CN')
    expect(screen.getByText('Starting system-selected speech (may be online)...')).toBeInTheDocument()
    act(() => synthesis.speak.mock.calls[0][0].onend?.())
    fireEvent.click(screen.getByRole('button', { name: 'Test English voice' }))
    expect(synthesis.speak).toHaveBeenCalledTimes(2)
    expect(synthesis.speak.mock.calls[1][0].voice).toBeUndefined()
    expect(synthesis.speak.mock.calls[1][0]).toMatchObject({ lang: 'en-US', rate: 1 })
    act(() => synthesis.speak.mock.calls[1][0].onend?.())
    expect(fetch).not.toHaveBeenCalled()
    expect((await db.preferences.get('workspace'))?.speechVoices ?? {}).toEqual({})
  })
})
