...
function SpeechConnectionForm({ connection, onSaved, busy, onBusyChange }: ConnectionSettingsProps & { connection?: SpeechConnection; onSaved: (notice: string) => void }) {
  ...
  return <form className="panel settings-form" aria-label="Practice speech connection" onSubmit={event => { event.preventDefault(); void perform('save') }}>
    ...
    <fieldset className="connection-fields" disabled={pending || busy}>
      ...
      <p className="small muted">Use a restricted key. Profile YAML exports include this key in plaintext; keep them private. Raw audio is never included. Without this connection, practice compares the recognized transcript locally rather than assessing pronunciation.</p>
    </fieldset>
    ...
    <div className="button-row">
      <button type="submit" className="button primary" disabled={pending || busy}>Save speech connection</button>
      {connection && <button type="button" className="button secondary" disabled={pending || busy} onClick={() => setRemoving(true)}>Remove speech connection</button>}
    </div>
    {removing && <div className="notice"><p>Remove this device's saved Azure Speech key and connection? Conversations, practice results, learning progress, and the AI connection will remain.</p>
      <div className="button-row">
        <button type="button" className="button secondary" disabled={pending || busy} onClick={() => void perform('remove')}>Confirm speech connection removal</button>
        <button type="button" className="button secondary" disabled={pending || busy} onClick={() => setRemoving(false)}>Keep speech connection</button>
      </div>
    </div>}
...
export function SpeechConnectionSettings({ busy, onBusyChange }: ConnectionSettingsProps = {}) {
  ...
  return <section aria-label="Speech connection settings">
    ...
    {import.meta.env.DEV && import.meta.env.DEV_LOCAL_SETTINGS === 'true' && <p className="small muted">The default profile can independently load speechConnection from {LOCAL_SETTINGS_FILE} once during initial setup when no speech connection is saved. Later file changes require an explicit profile import; removing a connection here keeps it removed across reloads.</p>}
    <SpeechConnectionForm key={saved.connection?.revision ?? 'unconfigured'} connection={saved.connection} onSaved={setNotice} busy={busy} onBusyChange={onBusyChange} />
