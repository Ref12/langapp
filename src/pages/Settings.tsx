...
export function Settings({ workspace, busy, run }: PageProps) {
  ...
  return <>
    ...
    <section className="panel"><h2>About this checkpoint</h2><p>The real 30-level Mandarin course map, with beginner levels 1-4 available for small lessons, grammar reference, and reading-recognition practice. Assistant supports connected text conversation and Shadow with browser Hear playback. Hear uses your saved voice selections; Automatic prefers local voices and uses online browser voices when needed, sharing the spoken text with that voice service.</p>
      <p>Translation practice defaults to no recording. With Listen and record selected, Conversation's Practice button starts recording and Submit shows feedback inline; Shadow uses Start speaking and automatically adds feedback when recording finishes. Azure Speech receives the audio and expected phrase only when Speech feedback is enabled and a connection is configured. Otherwise, the app compares a browser-recognized transcript locally; the browser may use an online recognition service. Results never go to the AI tutor, including in later turns. Raw recordings are never stored.</p>
      <p>Speech assessments are practice feedback, not evidence of mastery or official HSK certification. General voice conversation, generated activities, and handwriting assessment are not connected.</p>
