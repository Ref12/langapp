...
describe('curriculum experience', () => {
  it('opens the whole lesson from its unit and keeps self-check and audio separate from progress', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Guided audio lesson' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('link', { name: 'Lesson overview' }))
    await screen.findByRole('heading', { name: 'About this lesson' })
    await user.click(screen.getByRole('link', { name: 'Guided audio lesson' }))
    await screen.findByRole('button', { name: 'Start guided audio' })
    expect(window.location.hash).toBe(`#lesson/${curriculumLessons[0].id}/audio`)
    expect(screen.queryByRole('group', { name: 'Choose your lesson type' })).not.toBeInTheDocument()
    view.unmount()
    render(<App />)
    await screen.findByRole('button', { name: 'Start guided audio' })
    expect(screen.queryByRole('button', { name: 'Pause guided audio' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('link', { name: 'Lesson overview' }))
    await screen.findByRole('heading', { name: 'About this lesson' })
    expect(screen.getByRole('link', { name: 'Visual lesson' })).toBeInTheDocument()
  ...
  it('opens real grammar and finishes a persisted small lesson without passing the level checkpoint', async () => {
    ...
    expect((await db.words.toArray()).map(word => word.wordId).sort()).toEqual([...lesson.wordIds].sort())
    ...
    await go(`lesson/${lesson.id}/${firstWordPage}`)
  ...
  it('introduces one word per page, supports previous and reload, and does not award progress', async () => {
    ...
    await user.click(screen.getByRole('link', { name: 'Visual lesson' }))
    await screen.findByRole('heading', { name: getWord(lesson.wordIds[0]).native })
    expect(document.querySelectorAll('.word-card')).toHaveLength(1)
    expect(document.querySelector('.word-card details')).toBeNull()
    expect(screen.getByRole('navigation', { name: 'Lesson pages' })).toBe(document.querySelector('.lesson-study')!.lastElementChild)
    expect(screen.queryByRole('group', { name: 'Choose your lesson type' })).not.toBeInTheDocument()
  ...
  it('keeps later lessons paginated through words, grammar rules, and individual examples', async () => {
    ...
    expect(screen.queryByRole('button', { name: /Add to learning set|In your learning set/ })).not.toBeInTheDocument()
    expect(document.querySelector('.word-card details')).toBeNull()
    expect(screen.getByRole('navigation', { name: 'Lesson pages' })).toBe(document.querySelector('.lesson-player')!.lastElementChild)
  ...
  it('searches the v2 dictionary by pinyin or label and adds items to the knowledge set', async () => {
    ...
    await screen.findByRole('button', { name: /^Grammar \(\d+\)$/, pressed: true })
    const grammarSearch = screen.getByRole('searchbox', { name: 'Search dictionary' })
    await user.type(grammarSearch, 's-shi4-n--identity')
    await waitFor(() => expect(screen.getAllByRole('article')).toHaveLength(1))
