...
export function LessonDetail({ lesson, workspace, busy, run, page }: PageProps & { lesson: Lesson; page?: string }) {
  ...
  const practiceButton = <button className={`button ${current === total ? 'primary' : 'secondary'} full-width`} disabled={busy}
    onClick={() => void run(async () => navigate(`practice/${await startPractice('lesson', lesson.id)}`))}>
  ...
  return <div className="lesson-player">
    ...
    <nav className="lesson-pagination" aria-label="Lesson pages">
      {current > 1 ? <a className="button secondary" href={`#lesson/${lesson.id}/${current - 1}`}><ArrowLeft size={16} /> Previous</a>
        : <button className="button secondary" disabled><ArrowLeft size={16} /> Previous</button>}
      {current < total && <a className="button primary" href={`#lesson/${lesson.id}/${current + 1}`}>
        {current < lesson.wordIds.length ? 'Next word' : 'Next'} <ArrowRight size={16} /></a>}
    </nav>
